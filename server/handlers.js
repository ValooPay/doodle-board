...
const createUser = async (req, res) => {
    ...
    try {
        ...
        if(testSearchForEmail){
            ...
            timestamps.push(loginDate)
            const newUser = await db.collection(USERS_COLLECTION).insertOne({ _id, username, email, password, createdPosts, likedPosts, timestamps })
            res.status(200).json({
                status: 200,
                data: newUser
            })
...
const getUserLogin = async (req, res) => {
    ...
    try{
        ...
        await db.collection(USERS_COLLECTION).updateOne({_id: foundUser._id}, {$push: {timestamps: loginDate}})
        res.status(200).json({
            status: 200,
            data: foundUser,
            timestamp: loginDate
...
const getUserLoginCheck = async (req, res) => {
    const client = new MongoClient(MONGO_URI)
    ...
    try{
        ...
        if(!foundUser.timestamps.includes(timestamp)){
            return res.status(404).json({
                status: 401,
                message: "Invalid timestamp"
            })
        }
        ///// make use of currentdate and see if timestamp is too old
        res.status(200).json({
            status: 200,
            data: foundUser
        })
        ...
        console.error("Login expired", error)
        res.status(500).json({
            status: 500,
            message: error.message
        })
